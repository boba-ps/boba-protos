...
class ClientGadgetInfo$Type extends runtime_5.MessageType {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        if (message.campType !== 0)
            writer.tag(2, runtime_1.WireType.Varint).uint32(message.campType);
        /* uint64 guid = 3; */
        if (message.guid !== 0n)
            writer.tag(3, runtime_1.WireType.Varint).uint64(message.guid);
        /* uint32 owner_entity_id = 4; */
        if (message.ownerEntityId !== 0)
            writer.tag(4, runtime_1.WireType.Varint).uint32(message.ownerEntityId);
        ...
        if (message.asyncLoad !== false)
            writer.tag(6, runtime_1.WireType.Varint).bool(message.asyncLoad);
