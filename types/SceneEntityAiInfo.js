...
class SceneEntityAiInfo$Type extends MessageType {
    constructor() {
    ...
    internalBinaryWrite(message, writer, options) {
        ...
        for (let k of Object.keys(message.skillCdMap))
            writer.tag(3, WireType.LengthDelimited).fork().tag(1, WireType.Varint).uint32(parseInt(k)).tag(2, WireType.Varint).uint32(message.skillCdMap[k]).join();
        ...
        for (let k of Object.keys(message.aiThreatMap))
            writer.tag(5, WireType.LengthDelimited).fork().tag(1, WireType.Varint).uint32(parseInt(k)).tag(2, WireType.Varint).uint32(message.aiThreatMap[k]).join();
        /* map<uint32, uint32> skill_group_cd_map = 6; */
        for (let k of Object.keys(message.skillGroupCdMap))
            writer.tag(6, WireType.LengthDelimited).fork().tag(1, WireType.Varint).uint32(parseInt(k)).tag(2, WireType.Varint).uint32(message.skillGroupCdMap[k]).join();
        /* uint32 cur_tactic = 7; */
